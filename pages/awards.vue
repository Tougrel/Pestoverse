...
<template>
    <NuxtLayout name="default">
        <UForm :state="state" @submit="onSubmit" v-if="status === 'authenticated'"
            ...
            <UDivider icon="i-mdi-creation"
                      :ui="{ border: { base: 'border-primary-700 dark:border-primary-500' }}" />
            <div class="flex flex-col gap-2 p-4">
                <p class="text-lg font-bold text-primary-700 dark:text-primary-500">Suggestions</p>
                <div class="flex flex-row gap-2">
                    <UBadge
                        v-for="name in names.slice((page - 1) * items_to_show, ((page - 1) * items_to_show) + items_to_show)"
                        color="gray" size="lg" :label="name" />
                </div>
